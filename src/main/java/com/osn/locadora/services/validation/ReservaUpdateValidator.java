...
public class ReservaUpdateValidator implements ConstraintValidator<ReservaUpdate, ReservaUpdateDTO> {

	@Autowired
	private HospedagemService hospService;

	@Override
	public void initialize(ReservaUpdate ann) {
	}

	@Override
	public boolean isValid(ReservaUpdateDTO objDto, ConstraintValidatorContext context) {
		...
		Hospedagem hospedagem = hospService.find(objDto.getHospedagemId());
